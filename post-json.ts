...
const defaultRetryOfErr =
  (err: ErrorWithCode): boolean =>
    err && (
      err.code === 'ECONNRESET' ||
      err.code === 'ETIMEOUT'
    )
...
): Promise<{ code: number, headers: string[], json: unknown, buffer: Buffer }> => {
...
function postJsonRetry(url: string, json: any, options: {
  agentOfUrl?: (url: string) => any
  timeout?: number
  retry?: number
  retryDelay?: number
  retryOfErr?: (err: ErrorWithCode) => boolean
} = { retryDelay: defaultRetryDelay, retryOfErr: defaultRetryOfErr }): Promise<{code: number;headers: string[];json: any;buffer: Buffer;}> {
  const {
    retry,
    retryDelay = defaultRetryDelay,
    retryOfErr = defaultRetryOfErr,
    ...rest
  } = options
  const before = Date.now()
  return postJsonNoRetry(url, json, rest).catch((err: ErrorWithCode) => {

    const flushed = meta.get(err, 'flushed')
    const retry_ = typeof retry === 'undefined' ? flushed ? defaultFlushedRetry : defaultUnflushedRetry : retry
    ...
    if (retry_ && shouldRetry) {
      return sleep(Math.max(0, retryDelay - (Date.now() - before))).then(() => postJsonRetry(url, json, {
        ...rest,
        retry: retry_ - 1,
        retryDelay,
        retryOfErr
